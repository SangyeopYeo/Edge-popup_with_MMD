...
opt = load_parser()
c = 1 - np.exp((opt.freezing_period * opt.project_freq/opt.niter)*np.log(opt.sparsity))
...
###########
    ...
    if opt.algorithm == "gm":
        number_of_weights_to_prune = np.ceil(
            c * num_active_weights).astype(int)
        number_of_biases_to_prune = np.ceil(
            c * num_active_biases).astype(int)
    elif opt.algorithm == "imp":
        number_of_weights_to_prune = np.ceil(
            imp_c * num_active_weights).astype(int)
        number_of_biases_to_prune = np.ceil(
            imp_c * num_active_biases).astype(int)
    elif opt.algorithm == "global_ep":
        number_of_weights_to_prune = np.ceil(
            (1 - opt.sparsity) * num_active_weights).astype(int)
        number_of_biases_to_prune = np.ceil(
            (1 - opt.sparsity) * num_active_biases).astype(int)
    ...
    agg_bias_scores = torch.cat(
        active_bias_scores_list) if opt.bias else torch.tensor([])
    ...
    scores_threshold = torch.sort(
        torch.abs(agg_scores), descending=False).values[number_of_weights_to_prune-1].item()
    #print(scores_threshold)
    if opt.bias:
        bias_scores_threshold = torch.sort(
            torch.abs(agg_bias_scores), descending=False).values[number_of_biases_to_prune-1].item()
    else:
    ...
    if update_thresholds_only:
        for layer in (conv_layers + linear_layers):
            ...
            layer.flag.data, layer.bias_flag.data = GetSubnet.apply(layer.scores.abs(), layer.bias_scores.abs(), opt.sparsity, scores_threshold, bias_scores_threshold)    

    else:
        for layer in (conv_layers + linear_layers):
            if opt.algorithm == "ep":
                layer.flag.data, layer.bias_flag.data = GetSubnet.apply(layer.scores.abs(), layer.bias_scores.abs(), opt.sparsity)
            else:
                layer.flag.data = ((layer.flag.data + torch.gt(layer.scores.abs(),  # TODO
                                    torch.ones_like(layer.scores)*scores_threshold).int() == 2).int()).float()
                if update_scores:
                ...
                if opt.bias:
                    layer.bias_flag.data = ((layer.bias_flag.data + torch.gt(layer.bias_scores, torch.ones_like(
                        layer.bias_scores)*bias_scores_threshold).int() == 2).int()).float()
                    if update_scores:
...
def get_layer_sparsity(layer, threshold=0):
    ...
    if opt.bias:
        eff_bias_scores = layer.bias_scores * layer.bias_flag #layer.bias_scores * layer.bias_flag
    num_middle = torch.sum(torch.gt(eff_scores, torch.ones_like(eff_scores)*threshold)
                            *torch.lt(eff_scores, torch.ones_like(eff_scores.detach()*(1-threshold)).int()))
...
def get_regularization_loss(model, regularizer='L2', lmbda=1, alpha=1, alpha_prime=1, device = None):
    ...
    def get_special_reg_sum(layer):
        ...
        reg_sum = torch.tensor(0.).to(device)
        w_i = layer.weight
        p_i = layer.scores
