...
class GetSubnet(autograd.Function):
    ...
    def forward(
        ...
        bias_scores,
        k,
        scores_prune_threshold=-np.inf,
        bias_scores_prune_threshold=-np.inf,
    ):
        ...
        if opt.algorithm == "ep":
            ...
            out = torch.gt(
                scores, torch.ones_like(scores) * scores_prune_threshold
            ).float()
            bias_out = torch.gt(
                bias_scores, torch.ones_like(bias_scores) * bias_scores_prune_threshold
            ).float()
...
class SubnetLinear(nn.Linear):
    def __init__(self, *args, **kwargs):
        ...
        if opt.algorithm not in ["tr", "imp"]:
            ...
            if opt.training_loss == "ADV":
                nn.init.xavier_uniform(self.weight.data, 1.0)
            else:
                pass
    ...
    def forward(self, x):
        if opt.algorithm == "gm":
            subnet, bias_subnet = GetSubnet.apply(
                self.scores,
                self.bias_scores,
                opt.sparsity,
                opt.threshold,
        ...
        if opt.algorithm == "ep":
            subnet, bias_subnet = GetSubnet.apply(
                self.scores.abs(), self.bias_scores.abs(), opt.sparsity
            )
            subnet = subnet * self.flag.data.float()
            bias_subnet = bias_subnet * self.bias_flag.data.float()
        ...
        if opt.algorithm in ["global_ep", "global_ep_iter"]:
            subnet, bias_subnet = GetSubnet.apply(
                self.scores.abs(),
                self.bias_scores.abs(),
                0,
                self.scores_prune_threshold,
                self.bias_scores_prune_threshold,
            )
...
class ResnetG(nn.Module):
    def __init__(
        self, nz, nc, ndf, imageSize=32, adaptFilterSize=False, useConvAtSkipConn=False
    ):
        ...
        if adaptFilterSize == True and useConvAtSkipConn == False:
            ...
            logger.warn(
                "WARNING: In ResnetG, setting useConvAtSkipConn to True because adaptFilterSize is True."
            )
        ...
        firstL = SubnetConvT(
            nz, int(filterSizePerLayer[0] * channel_multiplier), 4, 1, 0, bias=False
        )
        #        nn.init.xavier_uniform(firstL.weight.data, 1.)
        lastL = SubnetConv(
            int(filterSizePerLayer[-1] * channel_multiplier),
            nc,
            3,
            stride=1,
            padding=1,
        ...
        for i in range(numUpsampleBlocks):
            nnLayers["resblock_%d" % i] = ResidualBlockG(
                filterSizePerLayer[layerNumber - 1],
                filterSizePerLayer[layerNumber],
                stride=2,
                useConvAtSkipConn=useConvAtSkipConn,
            )
            layerNumber += 1
        nnLayers["batchNorm"] = nn.BatchNorm2d(
            int(filterSizePerLayer[-1] * channel_multiplier), affine=opt.affine
        )
...
class ResidualBlockG(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1, useConvAtSkipConn=False):
        ...
        self.conv1 = SubnetConv(
            int(in_channels * channel_multiplier),
            int(out_channels * channel_multiplier),
            3,
            1,
            padding=1,
            bias=opt.bias,
        )
        self.conv2 = SubnetConv(
            int(out_channels * channel_multiplier),
            int(out_channels * channel_multiplier),
            3,
            1,
...
class SNGenBlock(nn.Module):
    def __init__(
        ...
        self.c1 = SubnetConv(
            in_channels, hidden_channels, kernel_size=ksize, padding=pad, bias=opt.bias
        )
        self.c2 = SubnetConv(
            hidden_channels, out_channels, kernel_size=ksize, padding=pad, bias=opt.bias
        )

        self.b1 = nn.BatchNorm2d(in_channels, affine=opt.affine)
        self.b2 = nn.BatchNorm2d(hidden_channels, affine=opt.affine)
        if self.learnable_sc:
            self.c_sc = SubnetConv(
                in_channels, out_channels, kernel_size=1, padding=0, bias=opt.bias
            )
...
class SNGenerator(nn.Module):
    def __init__(self, activation=nn.ReLU(), n_classes=0):
        ...
        if opt.First_freeze == True:
            self.l1 = FixedSubnetLinear(
                128, (self.bottom_width**2) * self.ch, bias=opt.bias
            )
        else:
            self.l1 = SubnetLinear(
                128, (self.bottom_width**2) * self.ch, bias=opt.bias
            )  # (args.latent_dim, (self.bottom_width ** 2) * self.ch, bias = opt.bias)
        self.block2 = SNGenBlock(
            self.ch, self.ch, activation=activation, upsample=True, n_classes=n_classes
        )
        self.block3 = SNGenBlock(
            self.ch, self.ch, activation=activation, upsample=True, n_classes=n_classes
        )
        self.block4 = SNGenBlock(
            self.ch, self.ch, activation=activation, upsample=True, n_classes=n_classes
        )
        self.block5 = SNGenBlock(
            self.ch, self.ch, activation=activation, upsample=True, n_classes=n_classes
        )
        self.b5 = nn.BatchNorm2d(self.ch, affine=opt.affine)
        if opt.Last_freeze == True:
            self.c5 = FixedSubnetConv(
                self.ch, 3, kernel_size=3, stride=1, padding=1, bias=opt.bias
            )
        else:
            self.c5 = SubnetConv(
                self.ch, 3, kernel_size=3, stride=1, padding=1, bias=opt.bias
...
class SNOptimizedDisBlock(nn.Module):
    def __init__(self, in_channels, out_channels, ksize=3, pad=1, activation=nn.ReLU()):
        ...
        self.c1 = SubnetConv(
            in_channels, out_channels, kernel_size=ksize, padding=pad, bias=opt.bias
        )
        self.c2 = SubnetConv(
            out_channels, out_channels, kernel_size=ksize, padding=pad, bias=opt.bias
        )
        self.c_sc = SubnetConv(
            in_channels, out_channels, kernel_size=1, padding=0, bias=opt.bias
        )
...
class SNDiscriminator(nn.Module):
    def __init__(self, activation=nn.ReLU()):
        ...
        self.block2 = SNDisBlock(
            self.ch, self.ch, activation=activation, downsample=True
        )
        self.block3 = SNDisBlock(
            self.ch, self.ch, activation=activation, downsample=False
        )
        self.block4 = SNDisBlock(
            self.ch, self.ch, activation=activation, downsample=False
        )
